...
public class PotionView {
    public static boolean view(Hero curHero) {
        while (true) {
            ...
            if (comm == 1) {
                while (true) {
                    ArrayList<Potion> potions = curHero.getInventory().getAllPotions();
                    ColorPrint.query("Please select a potion to consume, or enter [0] to cancel and go back");
                    int index = UserInput.getInstance().getChoice(0, potions.size());
                    if (index == 0) {
                        break;
                    }
                    Potion curPotion = potions.get(index - 1);
